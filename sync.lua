...
------------------------------------------------------------
-- PACKET HANDLING (BUFF TRACKING & ZONING RESET)
------------------------------------------------------------
ashita.events.register('packet_in', 'HandlePackets', function (e)
    -- Zoning Packet
    if (e.id == 0x00A) then
        for _, c in ipairs(chars) do
            -- Reset all toggles EXCEPT Follow (f)
            c.e[1] = false
            c.hs[1] = false
            c.bs[1] = false
            c.qs[1] = false
...
local function handle_buffs(caster, now)
    ...
    -- Pre-scan: Is anyone actually requesting buffs?
    local buffs_requested = false
    for _, c in ipairs(chars) do
        if c.active and c.buf[1] then
            buffs_requested = true
            break
        end
    end

    -- 1. COMPOSURE: Only fire if at least one person has 'Bu' ON
    if buffs_requested and not has_buff(caster, 419) then
...
local function handle_buffs(caster, now)
    ...
    -- Pre-scan: Is anyone actually requesting buffs via the 'Bu' button?
    local buffs_requested = false
    for _, c in ipairs(chars) do
        if c.active and c.buf[1] then
            buffs_requested = true
    ...
    -- 1. COMPOSURE: Only fire if at least one person has the toggle ON
    if buffs_requested and not has_buff(caster, 419) then
        if now - (caster.last_cast[419] or 0) > 10 then 
            qcmd(string.format('/mst %s /ja "Composure" <me>', caster.name))
            caster.last_cast[419] = now; caster.action_lock = now + 2.0; return true
    ...
    -- 2. PARTY BUFFS: Logic remains same (checks has_buff inside)
    for _, c in ipairs(chars) do
        if c.active and c.buf[1] then
            local target = (c.name_lower == caster.name_lower) and "<me>" or c.name
            local buffs = { 
