        ...
        if party:GetMemberIsActive(p) == 1 then
            ...
            if tIdx ~= 0 then 
                ...
                if not found then
                    ...
                    init_char(guest) 
                    guest.job, guest.sId, guest.pIdx = party:GetMemberMainJob(p), party:GetMemberServerId(p), p
                    table.insert(chars, guest)
                end
            end
        end
    end
    -- Cleanup inactive guests (e.g. they left party or zoned)
...
------------------------------------------------------------
local function handle_buffs(caster, now)
    -- GLOBAL SAFETY: If the caster's own 'Bu' button is off, 
    -- we kill the logic here. No Composure, no party buffs.
    if not caster or not caster.buf[1] or now < load_delay then 
        return false 
    end

    -- 1. COMPOSURE: Only cast if missing AND button is checked
    local caster_buffs = global_buff_cache[caster.sId]
    if caster_buffs and not caster_buffs[419] then
        if now - (caster.last_cast[419] or 0) > 30 then 
            qcmd(string.format('/mst %s /ja "Composure" <me>', caster.name))
            caster.last_cast[419] = now; caster.action_lock = now + 2.0; return true
        end
    end
    ...
    -- 2. PARTY BUFFS
    for _, c in ipairs(chars) do
        -- This ensures she only buffs people who have THEIR button checked
        if c.active and c.buf[1] then
            local target = (c.name_lower == caster.name_lower) and "<me>" or c.name
            
            -- Buff definition (ID, SpellName, JobRestricted)
            local buffs = { 
                {id=33,  name="Haste II"},    
                {id=43,  name="Refresh III", job=true}, 
                {id=116, name="Phalanx II"}   
            }
            ...
            for _, b in ipairs(buffs) do
                if (not b.job or REFRESH_JOBS[c.job]) and not has_buff(c, b.id) then
                    ...
                    local final_spell = b.name
                    if b.id == 116 then
                        final_spell = (c.name_lower == caster.name_lower) and "Phalanx" or "Phalanx II"
                    end

                    if c.sId ~= 0 then
                        qcmd(string.format('/mst %s /ma "%s" %s', caster.name, final_spell, target))
                        c.last_cast[b.id] = now; caster.action_lock = now + CAST_LOCK; return true
                    end
                end
            end
...
------------------------------------------------------------
-- RDM ACTION LOGIC (v1.1.17)
------------------------------------------------------------
local function handle_buffs(caster, now)
    if not caster or now < load_delay then return false end

    -- 1. COMPOSURE: Only cast if missing AND not recently used
    local caster_buffs = global_buff_cache[caster.sId]
    if caster_buffs and not caster_buffs[419] then
        if now - (caster.last_cast[419] or 0) > 300 then
            qcmd(string.format('/mst %s /ja "Composure" <me>', caster.name))
            caster.last_cast[419] = now; caster.action_lock = now + 2.0; return true
        end
    end
    ...
    for _, c in ipairs(chars) do
        if c.active and c.buf[1] then
            ...
            -- DEFINITION: {id, spell_name, needs_job_check}
            local buffs = { 
                {id=33,  name="Haste II"},    -- Haste ID is 33
                {id=43,  name="Refresh III", job=true}, -- Refresh ID is 43
                {id=116, name="Phalanx II"}   -- Phalanx ID is 116
            }

            for _, b in ipairs(buffs) do
                -- Logic: Only cast if the buff is missing from the server cache 
                -- AND we haven't cast it ourselves in the last 3 minutes (safety)
                if (not b.job or REFRESH_JOBS[c.job]) and not has_buff(c, b.id) then
                    
                    -- SPELL SELECTION FIX:
                    -- If the target is the caster, use "Phalanx". 
                    -- If the target is anyone else, use "Phalanx II".
                    local final_spell = b.name
                    if b.id == 116 and c.name_lower ~= caster.name_lower then
                        final_spell = "Phalanx II"
                    elseif b.id == 116 then
                        final_spell = "Phalanx"
                    end
...
local function handle_buffs(caster, now)
    ...
    local caster_buffs = global_buff_cache[caster.sId]
    if caster_buffs and not caster_buffs[419] then
        if now - (caster.last_cast[419] or 0) > 30 then -- Increased window to prevent spam
            qcmd(string.format('/mst %s /ja "Composure" <me>', caster.name))
            caster.last_cast[419] = now; caster.action_lock = now + 2.0; return true
        end
    end
