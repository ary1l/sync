...
local PRO_SHELL_DELAY     = 180.0 -- Prevent spamming long-duration buffs
...
local function force_sync()
    ...
    for i = 0, 17 do
        if p:GetMemberIsActive(i) == 1 then
...
ashita.events.register('packet_in', 'HandlePackets', function (e)
    if (e.id == 0x076) then
        ...
        for k = 0, 4 do
            ...
            -- Safely unpack Little-Endian ID to avoid bitwise corruption
            local sId = struct.unpack('I', e.data, memberOffset + 1) 
            
            if sId and sId > 0 then
                local rawBuffs = {}
                -- FFXI 0x076 holds the lower 8-bits of the 32 party buffs starting at byte 16
                for i = 0, 31 do
                    local bid = data[memberOffset + 16 + i + 1] 
                    if bid ~= nil and bid > 0 and bid ~= 255 then 
                        rawBuffs[bid] = true 
                    end
                end
...
local function handle_buffs(caster, now)
    ...
    for _, c in ipairs(chars) do
        if c.active and c.buf[1] then
            ...
            -- Refresh III
            if REFRESH_JOBS[c.job] and not has_buff(c, 591) then
                if now - (c.last_cast[591] or 0) > SELF_TRUST_DURATION then
            ...
            -- Protect V / Shell V (Using extended delay to prevent spam)
            if not (has_buff(c, 40) or has_buff(c, 41)) then
                if now - (c.last_cast[40] or 0) > PRO_SHELL_DELAY then
            ...
            if not (has_buff(c, 42) or has_buff(c, 43)) then
                if now - (c.last_cast[42] or 0) > PRO_SHELL_DELAY then
                    qcmd(string.format('/mst %s /ma "Shell V" %s', caster.name, target))
                    c.last_cast[42] = now; caster.action_lock = now + CAST_LOCK; return true
                end
            end
            ...
            if not has_buff(c, 116) then
                if now - (c.last_cast[116] or 0) > SELF_TRUST_DURATION then
                    local spell = (c.name_lower == caster.name_lower) and "Phalanx" or "Phalanx II"
...
ashita.events.register('d3d_present', 'logic_loop', function ()
    ...
    -- Heartbeat Request
    local any_buffing = false
    for _, c in ipairs(chars) do if c.buf[1] then any_buffing = true; break end end
    if any_buffing and now - (goomy.last_sync or 0) > 10.0 then
        force_sync()
        goomy.last_sync = now
    end
    ...
    for p = 0, 17 do
        if party:GetMemberIsActive(p) == 1 then
            ...
            if not found then
                local guest = { name=rawName, name_lower=pName, is_guest=true, active=true, pIdx=p, sId=party:GetMemberServerId(p), job=party:GetMemberMainJob(p), f={false}, e={false}, hs={false}, bs={false}, qs={false}, abs={false}, deb={false}, buf={false} }
        ...
        if not c.is_guest and i ~= 1 and c.active then
            ...
            if c.e[1] and mainEngaged then
                if c.engaged_target ~= effectiveTarget then
                    qcmd(string.format('/mst %s /attack [t]', c.name)); c.engaged_target = effectiveTarget
                    if i == RDM_INDEX then c.step, c.done, c.silence_done = 1, false, false end
                end
    ...
    -- Goomy Main Logic
    if now > goomy.action_lock and goomy.active then
        if mainEngaged and goomy.deb[1] and not goomy.done then
            ...
            if s then qcmd(string.format('/mst %s /ma "%s" [t]', goomy.name, s)); goomy.action_lock = now + CAST_LOCK; goomy.step = goomy.step + 1 
            else goomy.done = true end
        else handle_buffs(goomy, now) end
    end

    -- JA Logic (Dancers/DRK)
    for i, c in ipairs(chars) do
        if not c.is_guest and i ~= 1 and i ~= RDM_INDEX and now > c.action_lock and c.active then
            ...
            elseif mainEngaged then
                ...
                elseif tp >= STEP_TP_THRESHOLD and now > c.step_last + 12 then
                    local s = (c.bs[1] and (not c.qs[1] or c.next_step == 'bs')) and "Box Step" or "Quick Step"
                    if c.bs[1] or c.qs[1] then 
                        qcmd(string.format('/mst %s /ja "%s" [t]', c.name, s))
                        if c.bs[1] and c.qs[1] then c.next_step = (c.next_step == 'bs') and 'qs' or 'bs' end
        ...
        if imgui.BeginTable('T', 9, bit.bor(ImGuiTableFlags_Borders, ImGuiTableFlags_RowBg)) then
            ...
            for i, c in ipairs(chars) do
                if c.active then
                    ...
                    local is_restricted = (i == 1 or c.is_guest)
                    imgui.TableNextColumn(); if not is_restricted then imgui.Checkbox('##F'..i, c.f) else imgui.TextDisabled("-") end
                    imgui.TableNextColumn(); if not is_restricted then imgui.Checkbox('##E'..i, c.e) else imgui.TextDisabled("-") end
                    imgui.TableNextColumn(); if not is_restricted then imgui.Checkbox('##H'..i, c.hs) else imgui.TextDisabled("-") end
                    imgui.TableNextColumn(); if not is_restricted then imgui.Checkbox('##BS'..i, c.bs) else imgui.TextDisabled("-") end
                    imgui.TableNextColumn(); if not is_restricted then imgui.Checkbox('##Q'..i, c.qs) else imgui.TextDisabled("-") end
                    imgui.TableNextColumn(); if not is_restricted then imgui.Checkbox('##A'..i, c.abs) else imgui.TextDisabled("-") end
                    imgui.TableNextColumn(); if i == RDM_INDEX then imgui.Checkbox('##D'..i, c.deb) else imgui.TextDisabled("-") end
...
ashita.events.register('load', 'sync_load', function()
    qcmd('/ms followme on'); qcmd('/mso /ms follow on'); force_sync()
end)addon.name    = 'sync'
addon.author  = 'aryl'
addon.version = '0.9.8.7' 
...
local REFRESH_JOBS = { [3]=true, [4]=true, [5]=true, [7]=true, [8]=true, [15]=true, [16]=true, [21]=true, [22]=true }
local SILENCE_LIST = {   ['Shadow']=true, ['Vampyr']=true, ['Warlock']=true  }

local chars = {
    { name='shaymin',  active=true, f={false}, e={false}, hs={false}, bs={false}, qs={false}, abs={false}, deb={false}, buf={false} },
    { name='muunch',   active=true, f={true},  e={false}, hs={false}, bs={false}, qs={false}, abs={false}, deb={false}, buf={false} },
    { name='slowpoke', active=true, f={true},  e={false}, hs={false}, bs={false}, qs={false}, abs={false}, deb={false}, buf={false} },
    { name='goomy',    active=true, f={true},  e={false}, hs={false}, bs={false}, qs={false}, abs={false}, deb={false}, buf={false} },
}
...
local function init_char(c)
    ...
    c.f_prev, c.e_prev = c.f[1], c.e[1]
    c.step, c.done, c.action_lock = 1, false, 0
    c.last_target, c.silence_done = 0, false
    c.last_cast = {} 
...
------------------------------------------------------------
-- PACKET HANDLING & BUFF CHECK
------------------------------------------------------------
ashita.events.register('packet_in', 'HandlePackets', function (e)
    if (e.id == 0x076) then
        for k = 0, 4 do
            local memberOffset = (4 + (k * 48))
            local sId = ashita.bits.unpack_be(e.data_raw, memberOffset * 8, 32)
            if sId > 0 then
                local rawBuffs = {}
                for i = 0, 31 do
                    local bitPos = (memberOffset + 16) * 8 + (i * 10)
                    local bid = ashita.bits.unpack_be(e.data_raw, bitPos, 10)
...
local function has_buff(c, buff_id)
    if c.pIdx == 0 then
        local pBuffs = mm:GetPlayer():GetBuffs()
        for j = 0, 31 do if pBuffs[j] == buff_id then return true end end
    end
    local buffs = global_buff_cache[c.sId]
    return buffs and buffs[buff_id] == true
end
...
------------------------------------------------------------
local function handle_buffs(caster, now)
    if not caster.buf[1] then return false end
    if not has_buff(caster, 419) and (now - (caster.last_cast[419] or 0) > 10) then
        qcmd(string.format('/mst %s /ja "Composure" <me>', caster.name))
        caster.last_cast[419] = now; caster.action_lock = now + 2.0; return true
    end
    for _, c in ipairs(chars) do
        if c.active and c.buf[1] then
            ...
            if not has_buff(c, 33) and (now - (c.last_cast[33] or 0) > SELF_TRUST_DURATION) then
                qcmd(string.format('/mst %s /ma "Haste II" %s', caster.name, target_str))
                c.last_cast[33] = now; caster.action_lock = now + CAST_LOCK; return true
            end
            if (is_self or REFRESH_JOBS[c.job]) and not has_buff(c, 43) and (now - (c.last_cast[43] or 0) > SELF_TRUST_DURATION) then
...
ashita.events.register('d3d_present', 'logic_loop', function ()
    ...
    local targ = mm:GetTarget()
    local tIdx = targ and targ:GetTargetIndex(targ:GetIsSubTargetActive()) or 0
    local effectiveTarget = (tIdx ~= 0) and (ent:GetTargetedIndex(tIdx) ~= 0 and ent:GetTargetedIndex(tIdx) or tIdx) or 0
    local mainEngaged = (party:GetMemberTargetIndex(0) ~= 0 and ent:GetStatus(party:GetMemberTargetIndex(0)) == 1)

    if effectiveTarget ~= 0 and effectiveTarget ~= caster.last_target then
        caster.step, caster.done, caster.silence_done = 1, false, false
        caster.last_target = effectiveTarget
        caster.action_lock = 0 
    end
